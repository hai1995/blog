...
2. 找到 `[Seat:*]` 部分，在该部分下添加一行配置（若已有该部分，直接添加；若无，可自行添加 `[Seat:*]` 后再添加）：
...
3. 保存并退出编辑器：按 Ctrl+O 确认保存，按 Enter 确认路径，再按 Ctrl+X 退出 nano。
